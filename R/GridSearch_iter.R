GridSearch_iter <- function(object,
  ...
  for(k in 1:length(.knns)){
    ...
    for(j in 1:length(.resolutions)){
      ...
      GS_mat <- addIterationDataToGSMat(GS_mat, sObj_i_j, assay.name, a_index, res, nn, weights)
...
addIterationDataToGSMat <- function(GS_mat, sObj_i_j, assay.name, a_index, res, nn, weights){
